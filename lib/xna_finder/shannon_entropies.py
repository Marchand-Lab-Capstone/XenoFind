...
def shift_sequence(sequence, ref, cigar):
    """
    shift_sequences takes in a sequence,  reference position, and cigar and
...
def wrapper(s_p, c_f, direction, shannon_outputs, n=10, verbose = False): #change this function name 
    """
    ...
    for i in range(len(top_read_ids)):
        ...
        if (len(subset_df) >0):
            ...
            '''
            CHANGE THIS TO TAKE IN DIRECTION PROPERLY NOTE THAT FOR READS THAT HAVE BEEN UNFLIPPED FROM ALIGNMENT, WHAAT IS CONSIDERED A FORWARD AND REVERSE ALIGNMENT
            '''
    ...
    ent_csv_path = ent_df.to_csv(os.path.join(shannon_outputs,'shannon_entropies.tsv'), sep='\t')
    
    
    # return the dataframe of id's and entropies
    return ent_df, ent_csv_path

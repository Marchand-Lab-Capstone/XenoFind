...
def read_trim(bam_path):
    ...
    with pysam.AlignmentFile(sorted_bam_path, 'rb') as bamfile:
        ...
        for read in bamfile.fetch():
            ...
            # Get alignment sequence, reverse complement the reads if they are called as reverse strands 
            alignment_sequence = read.query_alignment_sequence if not read.is_reverse else str(Seq(read.query_alignment_sequence).reverse_complement())
...
def rename_consensus_headers(consensus_fasta_file, output_file):
    ...
    with open(output_file, 'w') as outfile:
        for i, record in enumerate(SeqIO.parse(consensus_fasta_file, "fasta"), start=1):
            ...
            new_header = f"Consensus{i}={read_id};size={size}"
...
def filter_cluster_size(fasta_path, threshold):
    ...
    output_path = os.path.join(os.path.dirname(fasta_path), 'final_consensus.fasta')
